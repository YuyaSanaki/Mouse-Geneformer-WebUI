...
def inhibit_tokens(example):
    ...
    [example["input_ids"].insert(insert_num, token) for token in example["tokens_to_perturb"][::-1]] 
    return example
# =====================================================================================
...
def active_indices(example):
    ...
    if any(isinstance(el, list) for el in indices):
        indices = flatten_list(indices)
    for index in sorted(indices, reverse=True):
        example["input_ids"].insert(example["shift_rank"], example["input_ids"].pop(index))
    return example
...
def make_perturbation_batch(example_cell, 
    ...
    if tokens_to_perturb == "all":
        if perturb_type in ["overexpress"]:
            ...
            if (perturb_rank_direct_shift is not None) and (perturb_rank_shift is None) :
                ...
                range_end = 0
            elif (perturb_rank_direct_shift is None) and (perturb_rank_shift is not None) :
                ...
                print("Error")
                print("You must choice one perturb shift type, direct(perturb_rank_direct_shift) or indirect(perturb_rank_shift). Not both.")
                sys.exit(1)
    ...
    if perturb_type == "delete":
        ...
        perturbation_dataset = perturbation_dataset.map(active_indices, num_proc=num_proc_i)
    elif perturb_type == "inhibit" :
        perturbation_dataset = perturbation_dataset.map(inhibit_indices, num_proc=num_proc_i)
    else :
        pass
    ...
    for i in range(0, total_batch_length, forward_batch_size):
        ...
        if perturb_type == "overexpress":
            ...
            if perturb_group == False :
                ...
                if actived_to_remove == 1 :
                    ...
                    minibatch_emb2 = minibatch_emb[:, actived_to_remove+len(tokens_to_perturb):, :]
                ...
                del minibatch_emb[:, perturb_rank_direct_shift:perturb_rank_direct_shift+len(tokens_to_perturb), :]
            else :
                pass
        elif perturb_type == "inhibit" :
            if perturb_group == False :
                inhibited_to_remove_start_index = mini_max_len - perturb_rank_direct_shift
                if inhibited_to_remove_start_index+len(tokens_to_perturb) == mini_max_len :
                    inhibited_to_remove_end_index = -len(tokens_to_perturb)
                    minibatch_emb = minibatch_emb[:, :inhibited_to_remove_end_index ,:]
                elif inhibited_to_remove_start_index+len(tokens_to_perturb) < mini_max_len :
                    minibatch_emb1 = minibatch_emb[:, :inhibited_to_remove_start_index, :]
                    minibatch_emb2 = minibatch_emb[:, inhibited_to_remove_start_index+len(tokens_to_perturb):, :]
                    minibatch_emb = torch.cat([minibatch_emb1, minibatch_emb2], dim=0)
                elif inhibited_to_remove_start_index+len(tokens_to_perturb) > mini_max_len :
                    print("perturb genes out of range.\
        ...
        if perturb_group == True:
            ...
            if len(tokens_to_perturb) == 1:
                indices_to_perturb_minibatch = [idx if idx != [-100] else [orig_max_len - len(tokens_to_perturb)]
                                                    for idx in indices_to_perturb[i:max_range]]
            else:
                num_perturbed = len(tokens_to_perturb)
                indices_to_perturb_minibatch = []
                end_range = [i for i in range(orig_max_len - num_perturbed, orig_max_len)]
                for idx in indices_to_perturb[i:max_range]:
                    if idx == [-100]:
                        ...
                        if len(idx) == num_perturbed :
                            ...
                            if len(idx) == 0 :
                                ...
                                print("num_perturbed: {}, len(idx): {}".format(num_perturbed,len(idx)))
...
class InSilicoPerturber:
    ...
    def validate_options(self):
        ...
        if self.perturb_type in ["inhibit", "activate"]:
            if (self.perturb_rank_shift is None) and (self.perturb_rank_direct_shift is None) :
                if self.perturb_type == "inhibit":
                    logger.warning(
                        "please set the perturb_rank_shift set into 1-4. " )
                    logger.warning(
                            "please set the perturb_rank_direct_shift set into 1-2048. " )
                elif self.perturb_type == "active":
                    logger.warning(
                        "please set the perturb_rank_shift into 1-4. " )
                    logger.warning(
                            "please set the perturb_rank_direct_shift into 1-2048. " )
                logger.warning(
                    "You must set the perturb_rank_shift. But not setting!!")
                logger.warning(
    ...
    def in_silico_perturb(self,
        ...
        if (self.tokens_to_perturb != "all") and (self.perturb_type != "overexpress") and (self.perturb_type != "active") and (self.perturb_type != "inhibit"): # delete の時に実行
        ...
        if self.perturb_group == True:
            ...
            def make_group_perturbation_batch(example):
                ...
                if self.perturb_type == "active" :
                    if (self.perturb_rank_direct_shift is not None) and (self.perturb_rank_shift is None) :
                        example["shift_rank"] = self.perturb_rank_direct_shift
                    elif (self.perturb_rank_direct_shift is None) and (self.perturb_rank_shift is not None) :
                        example["shift_rank"] = self.perturb_rank_direct_shift
                    else :
                        print("Error")
                        print("You must choice one perturb shift type, direct(perturb_rank_direct_shift) or indirect(perturb_rank_shift). Not both.")
