...
4. **ISP states** — pick **start_state** / **end_state** (e.g. AD, WT). **Genes to perturb** — enter mouse symbols (e.g. `Actb, Gapdh`) or Ensembl IDs; leave empty for genome-wide ISP. Then **Apply setting to Config YAML**.
...
Workflow and YAML fields: [docs/pipeline.md](docs/pipeline.md). Default ISP runs all genes (can take ~30 hours on DGX Spark). Use a single gene (e.g. `Actb`) for a quick test run.
